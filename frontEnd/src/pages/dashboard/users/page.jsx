...
export default function UserManagementPage() {
  ...
  const { user, isAuthenticated, isLoading, userList, roleUpdate, statusUpdate, deleteUser } = useAuth();
  ...
  const handleStatusToggle = async (userId, newStatus) => {
    try {
      const updatedUser = await statusUpdate(userId, newStatus);

      setUsers(prev => prev.map(u => u._id === updatedUser.id ? updatedUser : u));
      const statusText = newStatus === 'active' ? 'activated' : 'deactivated';
      toast.success(`${updatedUser?.name} has been ${statusText}`);
      fetchUsers(); // Refresh the user list to reflect status change
    } catch (error) {

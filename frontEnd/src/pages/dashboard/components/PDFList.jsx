...
export default function PDFList({ pdfs, onDelete, user }) {
  ...
  const handleDeleteClick = async (id) => {
    ...
    try {
      await axios.delete(`${API_URL}/admin/documents/${id}`, {
        withCredentials: true,
      });
      onDelete(id);
                  ...
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {user?.role === 'SUPERADMIN' ? (
                      <button
                        ...
                        className={`w-8 h-8 flex items-center justify-center rounded-lg transition cursor-pointer ${deleteConfirm === pdf.id
